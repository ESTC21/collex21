...
class ResultsController < ApplicationController
  ...
  def get_annotation
    ...
    annotations = Annotation.joins(:predicate).select('annotations.id, annotations.subject_uri, annotations.flag, annotations.object_uri, predicates.display_name')
    .where(annotations: {subject_uri: uri, flag: 1}).order('annotations.created_at desc')

    #path = Namespace.where('tablename = ? ', 'annotations').first.path

    #@annotations1 = Annotation.where(annotations: {predicate_id: 3}).where(annotations: {object_uri: 'https://estc21.lib.ucdavis.edu/annotations/1'}).count
    
    
    #results = []
    #annotations.each do |annotation| 
     # annotationPath = path + '/' + annotation.id.to_s
      #totalagree_disagree = Annotation.select("SUM(case when predicate_id = 3 then 1 else 0 end) as TotalAgree, SUM(case when predicate_id = 4 then 1 else 0 end) as TotalDisagree").where(annotations: {object_uri: annotationPath})
      ...
      #  where object_uri = 'https://estc21.lib.ucdavis.edu/annotations/1';
       ...
       # results.push({:predicate => annotation.display_name, :object => @solr.get_object(annotation.object_uri),
      ...
      #else
       # results.push({:predicate => annotation.display_name, :object => @solr.get_object(annotation.object_uri),
        #     :totalAgree => 0, :totalDisagree => 0, :annotationStatus => annotation.flag}) 
      #end
    #end
    
    respond_to do |format|
    format.json {
      #render json: { annotations: results, totalreviewitem: results.count }
      render json: { totalreviewitem: annotations.count }
    }
    end
  ...
  def get_annotation_feedback
    ...
    if feedback.present?
      ...
      annotationId = params[:annotationid] #Annotation.where('subject_uri = ? and object_uri = ?', params[:subjectUri], params[:objectUri]).first.id
      userId = get_curr_user_id()

      objectpath = Namespace.where('tablename = ? ', 'annotations').first.path
      subjectpath = Namespace.where('tablename = ? ', 'users').first.path

      subject = subjectpath +"/"+ userId.to_s
      object = objectpath +"/"+ annotationId.to_s 
      
      opinion = params[:opinion]  
      begin
        annotation = Annotation.new()
        annotation.subject_uri = subject
        annotation.flag = opinion
        annotation.user_id = userId
        annotation.feedback = feedback
        annotation.object_uri = object
        #annotation.predicate_id = predicate
        annotation.attachment_no = (lastrecord + 1) if params[:documents].present?
        annotation.save!
        ...
        flash = "OK:Feedback Saved!!"
      rescue Exception => msg
        flash = "**** ERROR: Saving feedback: " + msg
      end
    else
