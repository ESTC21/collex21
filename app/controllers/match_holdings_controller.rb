class MatchHoldingsController < ApplicationController
before_filter :init_view_options
  ...
  def match
    ...
    subjectUri = session[:parentUri]
    objectUri = session[:childUri]
    objectTitle = session[:childTitle]
    predicate = Predicate.find(1)
    predicateDisplayname = predicate.display_name
