...
function doViewAnnotation(row, viewAnnotation, subject_uri, url, row_num, row_id, is_logged_in, title)
{
	if (!is_logged_in) {
		var dlg = new SignInDlg();
		dlg.setInitialMessage("Please log in to check matches");
		dlg.setRedirectPageToCurrentWithParam('script=doCheckAnnotation&uri='+subject_uri+'&row_num='+row_num+'&row_id='+row_id);
		dlg.show('sign_in');
		return;
	}
	var html = "";
	
	if (viewAnnotation[0].innerHTML.indexOf("View Annotation") >= 0) {
		viewAnnotation[0].update(viewAnnotation[0].innerHTML.gsub("View Annotation", "Hide Annotation"));

		var params = {uri: subject_uri, url: url, row_num: row_num, full_text: '', title: title};
	var onSuccess = function(resp) {
		...
		{	
			html += window.pss.createHtmlTag("span", { 'class': 'label', 'style': 'font-weight: bold;'}, "[" + json.totalreviewitem + " active annotation]"); 
		}
		row.html(html);
	};

    var progressParam = { waitMessage: 'Collecting matches...' };
	serverAction({ action: { actions: "/results/get_annotation", els: [], params: params, onSuccess:onSuccess }, progress: progressParam});

	}
	else
	{
		viewAnnotation[0].update(viewAnnotation[0].innerHTML.gsub("Hide Annotation", "View Annotation"));
		row.html(html);
	}
}
...
function matchFeedbackinfo(annotationid){
	var title = "Provide your feedback";
	var existing_note = "";
	var populate_collex_obj_url = '/forum/get_nines_obj_list';
	 
	new feedbackDlg('/results/get_annotation_feedback', annotationid, [{text: 'Agree', value: '3'}, {text: 'Disagree', value: '4'}]); 
}
...
var titleLinkCounter = 0; // Just need a unique number, so we'll just keep counting here.
	function createTitleLink(title, url) {
		...
		if (title.length < 80)
			...
			var id = "match_title_more_" + titleLinkCounter;
...
function createResultHeader(obj) {
		...
		var titleEl = window.pss.createHtmlTag("div", { 'class': 'match_result_header' }, uriLink+a);
...
function getchooseObjectInfo(link) {
	...
	var checkboxes = Form.getInputs('bulk_collect_form', 'checkbox');  
	var selectelements = document.getElementsByClassName('annotation_predicateinfo');
	for (var i= 0; i < checkboxes.length; i++) {
		var checkbox = checkboxes[i];
	...
	if(objectsinfo.length == 0){		
		$j("#match_error").text("Please select at least one checkbox and provide the parent/child relationship using the dropdown menu next to the item!");
	}
	else{
		...
		serverAction({action:{actions: "/match/getMatchInfo", els: [], params: { objs: objectsinfo}, onSuccess:onSuccess}});		
	}	
}
